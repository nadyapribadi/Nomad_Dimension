...
| 1 Research (`stage-1`, nav 2) | tabs **YouTube · Prep · Places · Web** (`switchTab('res',…)`). YouTube: `renderChannelChips` · `addChannel` · `saveChannels` · `fetchYouTube` · `filteredYTVideos` · `renderYTVideos` · `resetYTFilters` · `openReviewModal` · `pushToNotion`. Prep: `loadSourceQueue` (→ `#ep-queue-area` picker) · `toggleEpVideo` (→ `renderPrepList`) · `clearEpSelection` · `renderPrepList` · `prepFetchTranscript` · `prepUsePasted` · `prepRunBreakdown` (structured JSON per video → `Content Outline`) · `parseStoredBreakdown` · `renderBreakdownText` · `rebuildCombinedOutline` (→ `S.episodeBreakdowns`). Places: `extractPlaces` · `enrichPlacesFromMaps` (→ `maps-proxy`) · `setPlaceField` · `renderPlacesReview` · `confirmPlace` · `skipPlace` · `renderConfirmedList` · `saveConfirmedPlaces`. Web: R4 stub. |
| 2 Episode Builder (`stage-2`, nav 3) | one tab **Assemble** (visible by default; `showStage(2)` primes `enterAssemble`). `buildIncludeList` / `renderIncludeList` (grouped by source, FROM: toggles) / `toggleInclude` / `moveInclude` / `toggleIncludeSource` / `groupSetChecked` · `suggestIncludeOrder` · `suggestTitles` · `draftAngle` · `episodeThemeOnly` · `updateEpisodePreview` · `createEpisode`. E1 folds in the typed-section browser. |
