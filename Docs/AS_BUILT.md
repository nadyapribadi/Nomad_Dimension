...
| `functions/maps-proxy.js` | `{query}` → Google Places API (New) Text Search. Returns prefecture / area / address / lat-lng / Maps URL / price tier for Stage 2 → Places enrichment. `GOOGLE_MAPS_API_KEY` optional; unset → `{found:false}`. Pure-fn helpers unit-tested |
...
Places DB columns: Name · Type · Prefecture · Price Range · Price Note · Halal ·
Country · B-Roll Description · Food · Google Maps URL · Address · Coordinates ·
Website · Rating · Review Count · Summary · Source Video · Used In Episodes ·
Date Added. **Notes** is left blank by `saveConfirmedPlaces` — it's the
operator's scratch field, every machine fact has its own column.
...
| Plan (nav 1 → `stage-7`) | `loadCalendar` · `runGapAnalysis` — calendar + "what's next" |
| 1 Research (`stage-1`, nav 2) | tabs **YouTube · Prep · Places · Web** (`switchTab('res',…)`). YouTube: channel/fetch/`pushToNotion` as before. Prep: **3-step strip** `prepStep(n)` (1 Pick · 2 Break down · 3 Review). `loadSourceQueue` (picker) · `toggleEpVideo`→`renderPrepList` · `prepFetchTranscript` · `prepUsePasted` · `prepRunBreakdown` (2-pass `EXTRACT_SHAPE_1`/`_2` via `runExtractCall`, gemini-flash; `stamp` adds `source_page_id`; **cache-first** — `_hash` (djb2 of transcript) on the stored breakdown skips the AI, `↻ Re-extract` forces it; a pass-2 failure saves pass-1 data but withholds `_hash` so the next run retries. `prepFetchAll`/`prepAnalyseAll` share `prepRunSeq` and recount pending-from-Notion for an honest end-of-run toast) · `renderExtractReview` → generic `renderReviewInto` (now a **table**, columns derived from each store's `props()` via `_propText`/`_esc`) / `poolFor` / `toggleReview` / `pushStore` over `RESEARCH_STORES` (activities·food·transport·research·prices·data·glossary). `Source Video` and `Used In Episode(s)` are Notion **relations** (`DUAL` back-relations) on the breakdown stores + Places (→ 🎞️ Source Videos / 🎬 Episodes); `_rel(source_page_id)`, web-sourced rows omit it. Review table flags rows already in the target DB (`ensureExisting`, cached in `S._existing`) — "in Notion" pill, unticked by default. Places: `extractPlaces` (own `PLACES_SHAPE` pass, carries `fromIds`) · `enrichPlacesFromMaps` (→ `maps-proxy`) · **`renderPlacesReview` is a table** (tick which to push, DB matches start unticked; `placeSkip` / `placesPick` / `queuePlaces`) · `saveConfirmedPlaces` (Source Video relation from `fromIds`). Web: `webMode` (ask / verify). `runWebResearch` = open question. Verify has two paths: `runWebVerify(store)` grounds this session's fresh Prep pool → `S.verify` decorations merged in `poolFor` (✅ + raised Confidence, no new rows); **`searchLibrary`** loads/keyword-searches rows already in the 8 Notion stores (`_readProp` flatten, client-filter; blank keyword = load all), `runLibVerify` grounds the ticked ones (batches of 4; splits 4→2→1 on timeout / malformed JSON / 5xx) and PATCHes each page's `Verify Status` + `Verify Note` (fact never overwritten). Verify cell renders via `_vlibVerifyCell` (verdict badge / `unfinished` / `— not checked`) and is repainted per-row mid-run by `_vlibPaintRow`; rows that don't finish keep `r._retry` and stay ticked. **`tidyVlib`** = deterministic dedup: `_clusterRows` (title Jaccard ≥ 0.5, `_normTokens`) + `_thinName`, pre-checks redundant/thin rows, `archiveVlib` sets `archived:true` (confirm-gated). Stores also have a manual `Notes` column. Both call `web-research` via `_verifyBatch`. `renderWebReview` shares the review engine (`src='web'`). |
| 2 Episode Builder (`stage-2`, nav 3) | one tab **Assemble** (`showStage(2)` primes `enterAssemble`). Identity: `suggestTitles` · `draftAngle` · `episodeThemeOnly` · `updateEpisodePreview`. Sections: `epTemplate` · `addSection`/`removeSection`/`moveSection` · `setSectionType` · `suggestSectionOrder` (✨). Material: `EP_MAT` per-category Notion query — `epLoadMaterial` · `renderMaterial` (search / prefecture / unused filters) · `assignItem` · `autoDistribute` (✨). `epGapCheck` (✨). `createEpisode` → Episode row + running-order brief in page body + one seeded Script section per typed section + `Used In Episode` stamps. State in `S.ep = { sections, assign, pool, _items }`. |
