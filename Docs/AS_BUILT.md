...
| 1 Research (`stage-1`, nav 2) | tabs **YouTube · Prep · Places · Web** (`switchTab('res',…)`). YouTube: channel/fetch/`pushToNotion` as before. Prep: `loadSourceQueue` (picker) · `toggleEpVideo`→`renderPrepList` · `prepFetchTranscript` · `prepUsePasted` · `prepRunBreakdown` (2-pass `EXTRACT_SHAPE_1`/`_2` via `runExtractCall`, gemini-flash) · `renderExtractReview` → generic `renderReviewInto`/`poolFor`/`toggleReview`/`pushStore` over `RESEARCH_STORES` (activities·food·transport·research·prices·data·glossary). Places: `extractPlaces` (own `PLACES_SHAPE` pass per transcript) · `enrichPlacesFromMaps` (→ `maps-proxy`) · `renderPlacesReview` · `saveConfirmedPlaces`. Web: `runWebResearch` (→ `web-research` fn) · `renderWebReview` (same review engine, `src='web'`). |
